...
def snakemake_workflow(tmp_path_factory) -> int:
    ...
    snakemake_cmd = f'snakemake --cores 4 --use-conda --directory {test_dir}/tests --snakefile {test_dir}/workflow/Snakefile --configfile {test_dir}/tests/config/config.yaml'
    ...
    process = subprocess.run(
        f'{snakemake_cmd}', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=test_dir
    )
